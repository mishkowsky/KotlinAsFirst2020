...
fun roman(n: Int): String {
    ...
    result.append(romanThousands[(n / 1000) % 10])
    result.append(romanHundreds[(n / 100) % 10])
    result.append(romanTens[(n / 10) % 10])
    result.append(romanUnits[n % 10])
...
fun hundred(n: Int, thousands: Boolean): List<String> {
    ...
    if (n / 100 % 10 != 0) list.add(hundreds[n / 100 % 10 - 1])
    if (n / 10 % 10 != 0) {
        list.add(
            if (n / 10 % 10 != 1) tens[n / 10 % 10 - 2]
            else tensUnderTwenty[n % 10]
        )
    }
    if (n % 10 != 0) {
        if ((n / 10 % 10 != 1) && thousands && (n % 10 in 1..2)) {
            list.add(if (n % 10 == 2) "две" else "одна")
        } else
            if (n / 10 % 10 != 1) list.add(units[n % 10 - 1])
    }
    return list
}
